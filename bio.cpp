...
int main()
{
    ...
    const float muFat = 0.193;  
    float* mu=(float*)malloc(sizeof(float)*Nx*Ny*Nz);                                                     //my for fat in scan
...
void readArr(float *x){
  string line;
  ifstream myfile ("headct.txt");
  int w,y,z;
  if (myfile.is_open())
  {
    while ( getline (myfile,line) )
    {
        fscanf(line, " (%d,%d,%d)", &w, &y,&z);
    }
    cout<<w<<y<<z;
    myfile.close();
  }
